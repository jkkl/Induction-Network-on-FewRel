...
# INTENT_DESC_OLD = "IntentDescOld"
# INTENT_DESC_NEW = "IntentDescNew"
#
# # INPUT/OUTPUT
...
# TRAIN_FILE_PREFIX = 'train_sample'
#
# # sheet name
...
class DataProcess:
    ...
    @staticmethod
    def get_sample_from_label_file(label_file, labels=[1]):
        '''
        从标注了正误的标注集中读取数据：
        ...
        if "Label" in label_data.columns:
            label_data = label_data[label_data['Label'].isin(labels)]
        label_data = label_data.apply(DataProcess.parse_row, axis=1)
        return label_data
    ...
    def parse_row(row):
        ...
        return pd.Series({REQUEST_CONTENT: request_content, INTENT_DESC: intent_desc, LABEL: label})
...
def get_test_sample_from_label_source(label_source_file_list, emotion_dim_map_file, intention_dim_map_file,
    ...
    target_dim = None
    if target_cat_dim_file:
        target_dim = load_data2df(target_cat_dim_file)
        pos_label_data = pos_label_data[pos_label_data[INTENT_DESC].isin(target_dim[INTENT_DESC])]

    neg_max_count = 5
    online_desc = pd.concat([emotion_dim_map[EMOTION_DESC], intention_dim_map[INTENT_DESC_96]])
    if target_dim is not None:
        # TODO: 为啥& 不是 且， neg_label_data = label_data[label_data[INTENT_DESC].isin(target_dim[INTENT_DESC]) & label_data[LABEL] == 0] \
        neg_label_data = label_data[label_data[INTENT_DESC].isin(target_dim[INTENT_DESC])][label_data[LABEL] == 0] \
    ...
    if target_dim is not None:
        random_neg_label_data = label_data[~label_data[INTENT_DESC].isin(target_dim[INTENT_DESC])][label_data[LABEL] == 0] \
    ...
    # 保存数据
    label_data_source_file = get_file_path_from_path(
        label_source_file_list[0]) + '\\' + TEST_FILE_PREFIX + f'_{pos_max_count}_{neg_max_count}.xlsx'
    with pd.ExcelWriter(label_data_source_file) as writer:
        sample_data.to_excel(writer, index=False)


def get_train_sample_from_label_source(label_source_file_list, intention_dim_map_all_file,
                                       mask_data_file):
    '''
    生成训练样本数据：从标注集合中
    '''
    ...
    label_data_list = []
    for label_data_file in label_source_file_list:
        label_data_list.append(DataProcess.get_sample_from_label_file(label_data_file, labels=[0, 1]))
    label_data = pd.concat(label_data_list)
    intention_dim_all = load_data2df(intention_dim_map_all_file)
    intention_map_all_dict = intention_dim_all.set_index(INTENT_DESC_OLD)[INTENT_DESC_NEW].to_dict()
    label_data[INTENT_DESC].replace(intention_map_all_dict, inplace=True)
    label_data.drop_duplicates(inplace=True)
    if mask_data_file:
        # 从label数据中mask掉指定数据
        mask_label_data = load_data2df(mask_data_file)
        label_data = label_data[~label_data[REQUEST_CONTENT].isin(mask_label_data[REQUEST_CONTENT])]

    # 保存数据
    label_data_source_file = get_file_path_without_suffix(
        label_source_file_list[0]) + '_' + TRAIN_FILE_PREFIX + f'.xlsx'
    with pd.ExcelWriter(label_data_source_file) as writer:
        label_data.to_excel(writer, index=False)
...
if __name__ == '__main__':
    ...
    '''
    data_dir = "C:\\Users\\User\\Documents\\project\\intention\\data\\test\\"
    label_data_dir = data_dir + "label_source\\"
    # data_dir = "D:\\project\\intention\\test\\"
    label_data_list = [
        # label_data_dir + "intention_test_labeled_data_all.xlsx",
        # # data_dir + "正向情绪index标签分类.xlsx",
        # label_data_dir + "intention_emotion_last7day.labeled.xlsx",
        # label_data_dir + "intention_64_1025-1031.sample.xlsx",
        # label_data_dir + "intention_19_1018-1024.sample.xlsx",
        # label_data_dir + "intention_4_1004-1010.sample.xlsx",
        # label_data_dir + "intention_label_manual.xlsx",
        # label_data_dir + "intention_neg_1106-1113.sample.xlsx",
        label_data_dir + "intention_134_label_origin.xlsx"
    ]
    ...
    # 获取训练数据
    mask_test_data_file = label_data_dir + "test_sample_10_5_unique.xlsx"
    # get_train_sample_from_label_source(label_data_list, intention_old_new_dim_map_file, mask_data_file=None)
    ...
    '''
        # Task 3 从测评数据中拉去96类别数据
        '''
    data_dir = "C:\\Users\\User\\Documents\\project\\intention\\data\\"
    dim_dir = data_dir + "test\\dim\\"
    # data_dir = "D:\\project\\intention\\test\\"
    label_data_list = [
        data_dir + "test_sample_10_5.xlsx"
    ]
